...
def solve(eq: str, dp=3) -> str:
    ...
    When exp and log are used, the procedure is slightly different but still generally the same. 
        ...
        - When log is found, the calculation is performed immediately, before the main calculation of the equation string, due to log having no binary operator 
          equivalent. The result is then placed back into the string, replacing log, and the function resumes as normal.
